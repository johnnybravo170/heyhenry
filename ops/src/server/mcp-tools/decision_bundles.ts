...
export function registerDecisionBundleTools(server: McpServer, ctx: McpToolCtx) {
  ...
  server.tool(
    ...
    [
      ...
      'blast_radius MUST lead with one bare token. Auto-shippable (PR, never merge):',
      'none | low | ui | copy | presentational | component | isolated. Review-gated',
      '(anything touching $/schema/auth/voice/design tokens): billing | schema | auth |',
      'voice | design-system | … . Lead with the token, optional prose after',
      '(e.g. "low — invoices only, reversible") — routing reads the leading token only.',
      'Omit it and the option routes to review.',
