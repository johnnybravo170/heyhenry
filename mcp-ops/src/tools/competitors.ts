...
export function registerCompetitorTools(server: McpServer) {
  server.tool(
    'competitors_list',
    'List tracked competitors, most-recently-updated first. Use to see who we already know about before deciding to add a new one.',
    {
      limit: z.number().int().min(1).max(500).default(50).describe('Max rows'),
    },
    async ({ limit }) => {
      try {
        const data = await opsRequest<{ competitors: Competitor[] }>(
          'GET',
          `/api/ops/competitors?limit=${limit}`,
        );
        const rows = data.competitors;
        if (rows.length === 0) return textResult('No competitors tracked yet.');
  ...
  server.tool(
    'competitors_get',
    'Fetch one competitor card by id, including the full edge_notes and latest_findings JSON.',
    { id: z.string().uuid() },
    async ({ id }) => {
      try {
        const data = await opsRequest<{ item: Competitor }>(`GET`, `/api/ops/competitors/${id}`);
        const c = data.item;
        let out = `${c.name}\n`;
        ...
        if (c.edge_notes) out += `Edge notes:\n${c.edge_notes}\n\n`;
  ...
  server.tool(
    ...
    async (input) => {
      try {
        const data = await opsRequest<{ id: string; url: string }>('POST', `/api/ops/competitors`, {
