...
export function registerDecisionTools(server: McpServer) {
  server.tool(
    'decisions_list',
    'List recorded decisions (a hypothesis + the action taken on it, then later the outcome). Use to find out whether something has already been tried before re-trying it.',
    {
      status: z
        .string()
        .optional()
    ...
    async ({ status, limit }) => {
      try {
        ...
        const data = await opsRequest<{ decisions: Decision[] }>(
          'GET',
          `/api/ops/decisions?${q.toString()}`,
        );
        const rows = data.decisions;
        if (rows.length === 0) return textResult('No decisions match.');
        let out = `${rows.length} decision(s):\n\n`;
        for (const d of rows) {
          out += `[${d.status}] ${d.title}\n`;
          out += `  ${formatDateTime(d.created_at)} by ${d.actor_name ?? 'unknown'}`;
          if (d.tags?.length) out += ` | #${d.tags.join(' #')}`;
          out += '\n';
  ...
  server.tool(
    'decisions_create',
    'Record a new decision: state the hypothesis ("X will improve Y") and the action being taken ("we are doing Z"). Later, attach an outcome with `decisions_record_outcome`.',
    {
    ...
    async (input) => {
      try {
        const data = await opsRequest<{ id: string; url: string }>('POST', `/api/ops/decisions`, {
  ...
  server.tool(
    ...
    'Append an outcome (with optional metrics) to a decision. Set `conclude=true` once you are done measuring — that flips the decision status to `learned` and stops further attention. Pass interim outcomes with `conclude=false`.',
    {
