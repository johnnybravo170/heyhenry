...
function readKey(): { keyId: string; secret: string; raw: string } {
  ...
  if (full) {
    ...
    if (u < 0) throw new Error('OPS_API_KEY malformed (missing secret separator)');
...
export async function opsRequest<T = unknown>(
  ...
  if (opts.reason) headers['x-ops-reason'] = opts.reason;
