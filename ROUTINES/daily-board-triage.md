...
## Step 4 — Ready arm: classify + tag (the dispatch loop executes)
Ready, no-decision cards are **classified and tagged here, not shipped here.** This routine never opens a PR — the single executor that holds git/gh/pnpm (`command-center-dispatch`) drains the lane. That closes the bypass where scout/Hermes cards filed straight to `todo` carried no `cc:` tag and so reached neither the executor nor a human decision. Apply the SAME blast-radius gate the Command Center uses (`ops/src/lib/blast-radius.ts`, `isLowBlast`):
- **Low-blast AND mechanical** — `isLowBlast` true *and* an established, no-novel-judgment pattern (e.g. pill → `<StatusBadge>`, off-scale token swap, a copy fix) → tag **`cc:autoship`**. Cap 3/run, leverage-ranked.
- **Higher-blast** (schema/migration · $/billing · auth · MCP surface · cross-tenant · shared `ui/*` primitive or design token · sacred-path) **OR low-blast but first-of-kind / novel** → tag **`cc:review`**.
- Doubt → downgrade to `cc:review`. Never upgrade.
**Tagging is not shipping.** `cc:autoship` only makes the card visible to the dispatch loop; whether it becomes a PR is governed by *that* loop's `ship_lane.auto_ship_mode` (`propose` = comment the plan, no code; `live` = branch → PATTERNS.md → typecheck → PR linking the card → mark `in-pr`; never merge). So this step is safe in any mode. One card = one tag; schema/workflow/$/sacred-path that needs a *call* still goes to Step 3 go/no-go, not here.
...
4. **Founding-cohort scope gate:** The now-context doc (Step 1) defines who the active founding cohort is and which verticals/customers are in scope right now. Cards and ideas that primarily serve a customer, tenant, or vertical *outside* the stated cohort fail the why-now gate and must **park** (`resurface:post-beta` or the relevant stage trigger) — not surface. Read the now-context before applying this; don't assume — the cohort will change as we progress and the gate should move with it. If a card touches an out-of-cohort surface AND exposes a genuine platform-wide gap (auth, billing, data integrity, shared infra), evaluate it on the platform-wide angle only and park the cohort-specific angle.
...
- **ONE digest email** — send via **`ops_digest_send`** (NOT `ops_email_send`; the formatter owns the HTML so the digest can't drift into a monospace wall). Pass STRUCTURED data only:
  ...
  - `top_item`: the single highest-leverage item, shown inline in full. `severity:"fire"` ONLY for genuinely broken-in-prod — requires (a) a confirmed Sentry incident / live outage with a link, OR (b) a dependency/SDK change where YOU have grep-confirmed a direct call site in the codebase (`file:line`). A deprecation notice or scout idea claiming impact without a confirmed call site does NOT qualify as a fire — treat it as a research item at most. The scout's "Confirmed affected surface" claim is not sufficient; triage must independently verify or downgrade. When in doubt, downgrade. `body` is markdown-lite — wrap model strings / file paths / env vars in `` `backticks` `` (renders as the ONLY monospace), `**bold**` for emphasis, blank lines for paragraphs.
