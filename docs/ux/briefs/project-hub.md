...
- **POs = lifecycle list** (draft→sent→acknowledged→received→closed via "Mark {next}"); open POs count toward Committed. **Gap (target):** the PO form has **no budget-category picker** → POs are created unbucketed; add one so they reconcile + show in By-category.
...
- **Henry (built) = the sub-quote OCR + bucket-allocation only.** Receipt-OCR, auto-categorize, and a missing-receipt nudge are **NOT built** — target enhancements, don't assume them.
