...
| Bank reconciliation (`/business-health/bank-*`) | ✅ `bank-reconciliation.md` | ✅ `od-bank-reconciliation` | 1 (build `d220e940`) | 🟡 (live, pre-redesign) | graduated from Business Health; render converged (1-round OD loop); **payment shortcut, not reconciliation** (QBO = book of record · "left for QBO by design") · status-tokens + glyph confidence bands · **✦ Henry deterministic matcher** + "how Henry matched" rubric · signed in/out glyph+tone · AlertDialog replaces `window.confirm` · loading/error states · Re-run matching · GST-inclusive matching · e-Transfer/Stripe/cheque parity. **Coding follow-up: owner+admin route gate** |
...
| Public pages (`/portal`, `/estimate`, `/approve`, `/decide`, `/view`, `/pulse`, `/home-record`) | ✅ `public-pages.md` | ✅ `od-public-pages` | 1 (build `f1f04673`) | 🟡 (live, pre-redesign) | render converged (1-round OD loop); **unified branded public-surface template** (GC letterhead, shared `<CustomerDocument>` shell) · portal hub (money-first · read-only Gantt · client-visible-only photos/docs · bottom-sheet tabs) · estimate-approval detailed+summary (`customer_view_mode` GC-only lever, GST tax-exempt-aware, mgmt-fee transparent, math ties out) · tap-to-decide SMS-link · states; client boundary clean (copy-refine the "what's private" explainer — on card). **Home Record → `home-record.md`; Pulse · Showcase · growth `/q` `/r` graduate** |
