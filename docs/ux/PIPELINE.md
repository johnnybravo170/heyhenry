...
| Referrals (growth) | ✅ `referrals.md` | ✅ `od-referrals` | 1 (build `7783050e`) | 🟡 (built, pre-Paper) | render converged (1-round OD loop); 2 surfaces — operator Refer&Earn (share-first cockpit · **✦ draft-the-note**, editable/never-auto-sent · status-tokens badges) + public `/r/[code]` landing (brand-light · "uses HeyHenry" pill · one rust CTA · **graceful invalid-code** · only `tenant_name`) · killed emerald/green → **rust sole accent** · **honest reward state — explicitly NOT faking "$300 earned"** (drop to Sent/Signups; reward+payout pipeline **graduates**) · client not homeowner. V1.1-candidate. |
