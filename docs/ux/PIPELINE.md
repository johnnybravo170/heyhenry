...
| Selections (per-room finishes) | ✅ `selections.md` | ✅ `od-selections` | 1 (build `97627958`) | 🟡 (live, pre-redesign) | render converged (1-round OD loop); **allowance-vs-actual variance surfaced** (labeled delta + room roll-up + over-allowance "Start CO" trigger + Henry nudge) · dual-authoring by-tags (operator/client/promoted) · idea→selection one-way promotion (distinct objects) · client portal view (allowance vs their actual — no margin/supplier/SKU/CO) · states; feeds Home Record. **Coding follow-up: wire CO-link to the real CO path** |
