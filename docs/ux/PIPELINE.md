...
| Overview | ✅ `overview.md` | ⚠ `desktop.html` **STALE** | 0 (`614a3b9a` ✅) | 🟡 #268/#301/#306 | aggregator engine + Paper restyle (#301); `schedule_slip` rule wired (#304); 3-layer badge consistency (#306: nav badges + strip derive from one `cache()`-shared `getProjectInsights`, bucketed by `owningTab` — issue-TYPE counts; Client unread stays item-count by design). **Only outstanding:** `ready_to_bill` rule (no clean "next draw" source yet). **subscreens: ✅ (overview.md)** · **Calm-typography pass (2026-06-01, OD-driver):** KPI `StatBox`es have an inverted hierarchy — mono-caps `<Eyebrow>` (~20× on this screen) out-shouts the hero numbers, which sit at 18px (`text-lg`, off the closed scale). Fix approved by Jonathan after before/after: StatBox number → `text-display-sm` 24px · `<Money>` whole-dollar summary mode (cents stay in line-item tables only) · **reserve `<Eyebrow>`** for KPI labels + section titles, demote sub-lines / table column-headers to quiet Inter `text-meta`. Design-system primitive pass (propagates hub-wide) → **dev card `614a3b9a`** ✅ shipped (incl. DESIGN.md + skill-clause sync + token-lint ratchet). · **⚠ OD render `desktop.html` is STALE** — predates this StatBox cockpit + the Committed→Projected Cost / Margin-at-Risk→Projected Margin relabel. Don't grade the build against it; **fresh Overview render = OD-Driver follow-up.** · **✅-gate unblocked** — run `heyhenry-design-critique` against live Overview + Budget at desktop AND mobile after PR #509 merges. |
...
- **Forms — cross-cutting Paper pass** (`forms.md`, 2026-05-23): the redesign restyled screen *chrome* but **not the forms/dialogs inside** — they're pre-Paper app-wide (low-contrast fields, ink-not-rust primary). **Root cause is shared primitives** (`ui/input` `bg-transparent` + warm-on-warm `--input` border; default `<Button>` ink), so **one primitive/token PR propagates to all ~40 forms** + a light per-form follow-up. This is the bulk of the "looks untouched" surface a manual click-through finds. **✅ Done (PR #509 / 2026-06-11):** fields `bg-card` + `shadow-xs` + `--input: #b5a48a`; `variant="primary"` rust added to Button; ~73 submit/CTA buttons swept app-wide. Updated PATTERNS.md §37 + DESIGN.md.
