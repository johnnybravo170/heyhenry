...
| Home Record (`/home-record/[slug]` + closeout flow) | ✅ `home-record.md` | ✅ `od-home-record` (operator + artifact) | 1 (build `26aa6be1`) | 🟡 (live, pre-redesign) | graduated from Photos/Docs/Notes + Public-pages; render converged (split into `operator-*` + `artifact-*` files — combined prompt too heavy for one run); **A** operator flow: button-soup → 3-step state strip + readiness (web/PDF/ZIP, stale guard) + preview drawer w/ **✦ Henry closeout-summary editor** + email-to-client dialog + regenerate; **B** public artifact on the `<CustomerDocument>` letterhead (summary as plain prose, behind-the-wall section, no-JS, 404) — **no ✦/Henry client-side**, no margin/cost. **Coding follow-ups: store Henry summary in snapshot · email via `renderEmailShell`** |
