...
| Settings ▸ Team (`/settings/team`) | ✅ `settings-team.md` | ✅ `od-settings-team` | 1 (build `925ee78a`) | 🟡 (live, pre-redesign) | graduated from Settings; render converged (1-round OD loop); two-region (Crew + Crew defaults) · role-led "Add to crew" invite (Worker/Admin/Member/Bookkeeper — +admin path) · collapsed + gated pay/charge (**margin OWNER-only**) · inline role editor + deactivate-not-delete (targets) · ✦ "GST # missing" roster flag · **no-seat guardrail**. **Coding follow-ups: `updateMemberRoleAction`+RLS · soft-deactivate+schema · admin-invite CHECK** |
