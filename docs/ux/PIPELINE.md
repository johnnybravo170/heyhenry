...
| Schedule | ✅ `schedule.md` | ✅ `…-schedule` | **1** (Henry) | 🟡 #295/#303/#304 | working-days (#303) + slip/digest/chrome (#304: shared `project-schedule-slip.ts` feeds digest + Gantt outline + Overview `schedule_slip` rule + tab badge; foot-guns → ⋯ + AlertDialog; per-bar quick actions; preview-as-customer); **outstanding:** Henry cascade/notify (`d0b1b72a`). ⚠ mobile Timeline reuses the scrollable Gantt behind the toggle (no bespoke compressed variant) — refinement candidate · **subscreens: ✅ (schedule.md)** |
