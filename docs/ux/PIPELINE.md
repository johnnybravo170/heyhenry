...
| Team checklists (`/checklists`) | ✅ `checklists.md` | ⬜ | — | 🟡 (live, pre-redesign) | cross-project crew-list rollup (`project_checklist_items`, **GC-native on `projects`**); embeds `TeamChecklist` (worker Today + hub + dashboard chip). Target: Paper restyle + **Tasks-vs-Checklist clarity**. QA-caught gap |
...
| Closeout gallery share (`/g/[handle]`) | ✅ `gallery-share.md` | ⬜ | — | 🟡 (live, pre-redesign) | token-gated public job-photo gallery + review/referral CTA; **job-keyed (PW) — the GC-on-`projects` path is the open scope Q**; adopt `<PublicBrandHeader>` (raw `neutral-*` today); internal tags stripped server-side. QA-caught gap |
