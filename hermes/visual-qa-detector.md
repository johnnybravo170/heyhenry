...
- **Credentials:** demo-tenant login — owner `gcdemo@example.com` (Maple Ridge Renos) or the Northbeam demo. Put the password in the profile's credential store, **NOT in this file** (canonical source is 1Password `op://Agents/gc-demo-password/password`, mirrored in the Ops vault "QA tenant credentials"). The seed script `scripts/setup-gc-demo-tenant.mjs` reads the same value from `$GC_DEMO_PASSWORD`; it no longer hardcodes it.
