...
- **App-shell header dividers don't meet at the corner** — on desktop, the sidebar header (brand + collapse row) and the main-content top bar each carry a bottom border that should land at the **same Y** and read as one continuous line across the top; if their heights differ, the line jogs where the vertical sidebar border crosses it (an "odd little corner") · "The line under the logo doesn't line up with the line under the top toolbar — there's a little step in the corner." · [auto if a one-off height mismatch; surface if it's a shell layout token] · **Measure it:** read `getBoundingClientRect().bottom` of the sidebar-header divider and the main-header divider — flag any difference (check both expanded and collapsed sidebar states). Root cause seen in the wild: the sidebar header was `h-12` while every other header is `h-14` (PR #450).
