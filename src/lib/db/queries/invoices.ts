 ...
 *
 * The implementation lives in `@/lib/invoices/totals` so client components
 * can import it without pulling `next/headers` (via `supabase/server`)
 * into the client bundle. New client-side callers should import from
 * there directly; this re-export keeps server-side callers stable.
 */
export { invoiceTotalCents } from '@/lib/invoices/totals';
