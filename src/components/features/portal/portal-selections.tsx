          ...
          <div key={group.room} className="rounded-lg border bg-card">
            ...
            <ul className="divide-y">
              {group.items.map((sel) => {
                ...
                // CLIENT BOUNDARY: client sees allowance vs THEIR actual only.
                // No margin/markup, no supplier, no SKU, no Start-CO (the
                // operator drives the CO). See the brief's CLIENT BOUNDARY.
                const variance = selectionVariance(sel.allowance_cents, sel.actual_cost_cents);
                return (
                  <li key={sel.id} className="px-4 py-3">
                    <div className="flex flex-wrap items-baseline gap-2">
                      ...
                      <ByTag createdBy={sel.created_by} selfLabel />
                    </div>
