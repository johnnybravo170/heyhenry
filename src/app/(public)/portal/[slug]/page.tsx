...
export default async function PortalPage({
  ...
  // Tab destinations in the brief's intentional order. `key` doubles as the
  // ?tab= query value ('project' is the bare /portal/[slug]). The bottom
  // sheet bar surfaces the thumb-reachable primaries; the full strip up top
  // keeps every tab reachable with no client JS.
  const TABS: Array<{
    key: typeof tab;
    label: string;
    badge?: number;
    ...
    { key: 'messages', label: 'Messages', badge: unreadFromBusiness },
  ];
  const tabHref = (key: typeof tab) =>
    key === 'project' ? `/portal/${slug}` : `/portal/${slug}?tab=${key}`;

  return (
    <TenantProvider timezone={tenantTz ?? 'America/Vancouver'}>
      {/* Warm Paper surface behind the floating white cards — the GC's
          letterhead, not flat white. pb-24 leaves room for the bottom bar. */}
      <div className="min-h-screen bg-background">
        <div className="mx-auto max-w-4xl px-4 pt-8 pb-24 sm:pb-8">
          {isOperatorPreview ? (
          ...
          {/* GC letterhead — the contractor's brand, never HeyHenry's. */}
          <div className="mb-5 overflow-hidden rounded-2xl border bg-card shadow-sm">
            <PublicBrandHeader
              logoUrl={logoUrl}
              businessName={businessName}
              context={customerName ? `${p.name as string} · ${customerName}` : (p.name as string)}
            />
            <div className="px-4 py-4">
              <h1 className="text-xl font-semibold tracking-tight">{p.name as string}</h1>
              <p className="mt-1 text-xs text-muted-foreground">
                {statusLabel} · {percentComplete}% complete
              </p>
            </div>
          </div>
          ...
          {tab === 'messages' ? (
            ...
            <>
              {/* Intentional order (brief): decisions pinned above the fold →
                status/progress → approvals → phases → docs → trades → updates. */}
              ...
              {/* Status + progress — the money/status anchor under the decisions. */}
              ...
              {/* Phase rail — milestone tracker. Read-only here; operator
          advances/regresses from the project detail Portal tab. */}
              {phases.length > 0 ? (
                <div className="mb-8">
          ...
          add their own picks. */}
              ...
              {/* Calm boundary note — reassures without spotlighting markup by
                enumerating what's withheld (a margin tell). */}
              <p className="mt-8 flex items-start gap-1.5 border-t pt-4 text-xs text-muted-foreground">
                <Shield aria-hidden className="mt-0.5 h-3 w-3 shrink-0" />
                <span>
                  Your portal shows everything we&rsquo;ve shared on your project — schedule,
                  photos, decisions, and contract figures.
                </span>
              </p>
            </>
          )}
        ...
        {/* Mobile bottom-sheet tab bar — thumb-reachable primaries + More
          (Budget/Selections/Ideas live in the scrollable top strip). Pure
          links, no client JS. Hidden on sm+ where the top strip suffices. */}
        <nav
          aria-label="Portal sections"
          className="fixed inset-x-0 bottom-0 z-30 flex border-t bg-card/95 pb-[env(safe-area-inset-bottom)] backdrop-blur sm:hidden"
        >
          {(() => {
            const bottomItems: Array<{
              ...
              { key: 'photos', label: 'Photos', Icon: ImageIcon },
              {
                ...
                label: 'Messages',
                Icon: MessageSquare,
                badge: unreadFromBusiness,
              },
            ];
            const moreActive = tab === 'budget' || tab === 'selections' || tab === 'ideas';
            return (
              <>
